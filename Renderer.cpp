...
Renderer::Renderer(QVulkanWindow *w, bool msaa)
    : mWindow(w), mCollisionAgent(*this)
{
    if (msaa) {
    ...
    mObjects.at(4)->setPosition(0, 50, 0);
...
void Renderer::initResources()
{
    ...
    mTextureHandle[0] = createTexture((assetPath + "pink.jpg")); //Heightmap.jpg HundA.bmp
    mTextureHandle[1] = createTexture((assetPath + "hund.bmp"));
...
void Renderer::startNextFrame()
{
    ...
    for (std::vector<VisualObject*>::iterator it=mObjects.begin(); it!=mObjects.end(); it++)
    {
        ...
        int texType = (*it)->getTextureType();
                      setTexture(mTextureHandle[texType], commandBuffer);
