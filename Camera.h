...
public:
    ...
    void update();
    void followPlayer(QVector3D &playerPosition, QVector3D &offset);
    void setPosition(const QVector3D& position);
    void pitch(float degrees);
    void yaw(float degrees);
    QVector3D cameraOffset{0.0f, 10.0f, 40.0f};
    ...
    QVector3D mManualOffset{0.0f, 0.0f, 0.0f}; // Your extra manual camera adjustments
    float mZoomDistance = 40.0f;
    float mYawOffset = 0.f;   // Orbit around player
    float mPitchOffset = 0.f; // Look up/down
